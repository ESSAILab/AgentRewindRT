...
4. **对照“原始请求”。** 确认智能体实际完成了什么，是否遗漏要求或修改了任务范围之外的文件。截图中原始请求只要求添加简短的 README 说明，可据此检查上方两个变更文件是否都有必要。
5. **核对“运行证据”和“执行命令”。** 工作区路径用于确认修改发生的位置；nono 会话标识、状态目录和 diff 摘要用于追溯对应的会话、快照与变更证据；执行命令用于核对本次运行实际启动的程序及参数。
6. **查看“nono 状态”和“记录摘要”。** 前者列出会话及回退数据目录等信息，不能单凭这些目录存在就认定回退成功；后者展示结构化记录，可进一步查看具体发现、意图分析理由和建议动作。较长的命令或记录可在内容框内滚动查看。
7. **选择人工处置。** 确认变更符合任务且风险可接受后，点击“接受变更”，记录人工放行并保留当前文件；需要恢复快照时，点击“执行回退”并确认。发出请求后应等待会话显示“已回退”，再检查工作区恢复结果；若显示“回退失败”，查看失败原因后处理。垃圾桶按钮只删除平台告警记录，不会回滚代码。
