...
以下两张截图展示同一条测试会话从分析结果到运行证据的完整审阅过程。点击告警卡片可展开详情，按“看结论、核对变更、追溯证据、人工处置”的顺序阅读。

![Web UI 会话审阅：统计卡片、风险与意图标签、变更风险分析及文件 diff](docs/assets/webui-1.png)

**先看风险结论，再核对实际变更。**

1. **定位待处理会话。** 顶部统计卡片展示待人工处理、已完成回退和总告警数。会话编号旁的标签分别表示风险等级、变更与任务的意图一致程度，以及人工处置状态。截图中的“严重风险”“部分超出”“待人工确认”表示：这次变更存在高优先级风险，部分内容超出原始任务，尚未完成人工处置。
2. **阅读“变更风险分析”。** 摘要说明风险来自哪里，右侧“拒绝变更”“建议回退”等标签给出分析裁决与建议。它们是审阅依据，不表示系统已经执行了回退。
3. **检查“本次变更”。** 逐个核对文件路径、变更类型、增删行数和深色区域中的 diff。截图中的任务是为 README 添加 `Agent Result` 小节，但会话还记录了一个额外的 `/tmp` 下 `.so` 文件，因此不能仅凭 README 修改正确就接受整次变更。应进一步确认额外文件的来源和用途；风险标签本身不等于已证实该文件具有恶意。若出现“diff 预览已截断”，还需查看完整变更后再判断。

![Web UI 会话审阅续图：原始请求、运行证据、执行命令、nono 状态和记录摘要](docs/assets/webui-2.png)

**继续向下核对任务与证据，最后作出处置。**

1. **对照“原始请求”。** 确认智能体实际完成了什么，是否遗漏要求或修改了任务范围之外的文件。截图中原始请求只要求添加简短的 README 说明，可据此检查上方两个变更文件是否都有必要。
2. **核对“运行证据”和“执行命令”。** 工作区路径用于确认修改发生的位置；nono 会话标识、状态目录和 diff 摘要用于追溯对应的会话、快照与变更证据；执行命令用于核对本次运行实际启动的程序及参数。
3. **查看“nono 状态”和“记录摘要”。** 前者列出会话及回退数据目录等信息，不能单凭这些目录存在就认定回退成功；后者展示结构化记录，可进一步查看具体发现、意图分析理由和建议动作。较长的命令或记录可在内容框内滚动查看。
4. **选择人工处置。** 确认变更符合任务且风险可接受后，点击“接受变更”，记录人工放行并保留当前文件；需要恢复快照时，点击“执行回退”并确认。发出请求后应等待会话显示“已回退”，再检查工作区恢复结果；若显示“回退失败”，查看失败原因后处理。垃圾桶按钮只删除平台告警记录，不会回滚代码。
